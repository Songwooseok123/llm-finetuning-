# ACCELERATE_USE_FSDP=1 FSDP_CPU_RAM_EFFICIENT_LOADING=1 torchrun --nproc_per_node=4 ./lora_multi_gemma.py --config ./config.yaml
...
def training_function(script_args, training_args):    
    ...
    train_dataset = train_valid["train"].select(range(2000))
    valid_dataset = train_valid["test"].select(range(30))
    ...
    tokenizer=tokenizer,
    #packing=True, ## collator 쓰려면 이거 주석처리하고 밑에 2줄 추가 
