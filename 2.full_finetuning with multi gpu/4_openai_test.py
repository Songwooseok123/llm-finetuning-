...
class Evaluation(BaseModel):
    ...
    completion_quality: Criterion
    average_score: float

def evaluate_qa_pair(idx, qa_pairs):
    client = get_openai_client()  # 각 프로세스마다 새로운 클라이언트 생성
    save_path = f"./qa_evaluation_results_254/result_{idx}.json"
...
6. 대답 완결성: 모델의 답변이 반복되거나 종결하지 않은 문장이지 않고, 완전한 문장인가? 
...
def main():
    ...
    data = open("./test/model_generation_result_without_col_checkpoint-254.txt", "r")
